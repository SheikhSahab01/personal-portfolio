...
const ExperienceCard = ({oneexperience}: Props) => {
  return (
    <article className=" bg-[#333333] p-10 snap-center flex flex-col items-center justify-start rounded-lg space-y-3 flex-shrink-0 w-[300px] md:w-[400px] xl:w-[600px] ">
        <motion.img className="h-24 w-28 rounded-md xl:w-[150px] xl:h-[100px] object-fill object-center " src={urlFor(oneexperience.companyImage).url()} alt="company_logo" />
        <div className="px-0">
            ...
            <div className="flex space-x-2 my-3">
              {oneexperience.technologies.map(tech => (
                <motion.img key={tech._id} className="h-7 w-7 rounded-full object-contain object-center" src={urlFor(tech.projectImage).url()} alt="" />
              ))}
            </div>

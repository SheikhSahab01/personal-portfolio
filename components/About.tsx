...
const About = ({pageInfo}: Props) => {
  return (
    <motion.div initial={{opacity: 0}} transition={{duration: 2}} whileInView={{opacity: 1}} className="flex flex-col h-screen relative text-center md:text-left md:flex-row max-w-7xl  px-10 justify-evenly mx-auto items-center gap-3">
        ...
        <motion.img initial={{opacity: 0}} transition={{duration: 2.5}} viewport={{once: true}} whileInView={{opacity: 1}} src={urlFor(pageInfo.profilePic).url()} alt="Personal_image" 
        className=" -mb-20 md:mb-0 flex-shrink-0 w-44 h-44 lg:w-100 lg:h-100 rounded-full object-cover object-top md:rounded-lg md:w-64 md:h-95 xl:h-[400px] xl:w-[300px] " />
